...
Proposed initial RF-managed release: **1.0.0**  
Status: **PREPARED / VALIDATION REQUIRED — UNPUBLISHED, NOT KNOWN-GOOD**  
...
`1.0.0` is the proposed initial RF-managed release version. The repository currently contains an unpublished candidate for that version. It must not be registered as an ACTIVE reusable clean ROLE in the Role Catalog until System Validation has actually PASSed, the RF Owner has explicitly approved the exact release candidate, and immutable release `v1.0.0` has been published.
